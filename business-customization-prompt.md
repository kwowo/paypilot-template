...
grep -r "src=\"https://" src/ | grep -v "localhost" | sed 's/.*src="https:\/\/\([^"]*\)".*/\1/' | cut -d'/' -f1 | sort -u > /tmp/external-domains.txt
echo "External image domains found (ensure all are configured in next.config.js):" && cat /tmp/external-domains.txt
...
**CRITICAL: NEVER MODIFY .ENV FILE**
```bash
...
- ✅ **External images:** All external domains configured in next.config.js remotePatterns
...
### Next.js Image Component Errors (Critical Fix)
```typescript
// ❌ Image components with empty/undefined src (causes console errors)
const [selectedImage, setSelectedImage] = useState<string>("");
// Later renders: <Image src={selectedImage} /> // Empty src causes errors

// ✅ Initialize with safe defaults and conditional rendering
const [selectedImage, setSelectedImage] = useState<string>(product?.image || "");

// Always use conditional rendering for images
{selectedImage ? (
  <Image src={selectedImage} alt={product.name} width={600} height={600} />
) : (
...
// ✅ Filter empty images from arrays
{product.images?.filter(img => img && img.trim() !== '').map((image, index) => (
  <Image key={index} src={image} alt={`View ${index + 1}`} />
))}
```
...
### Next.js External Image Configuration (Critical Fix)
```javascript
// ❌ Problem: Next.js Image component with external URLs fails without domain config
<Image src="https://images.unsplash.com/photo-123" alt="Product" width={300} height={200} />
// Error: "hostname not configured under images in your next.config.js"

// ✅ Solution: Configure external domains in next.config.js
const config = {
  images: {
    remotePatterns: [
      {
      ...
      // Add all external image domains you use
    ],
  },
};
...
// ✅ Always configure external image domains before using them
// Common external image sources that need configuration:
// - images.unsplash.com
// - cdn.shopify.com  
// - res.cloudinary.com
...
- [ ] **External Image Config:** All external domains configured in next.config.js remotePatterns
...
# ❌ NEVER modify existing .env variables:
